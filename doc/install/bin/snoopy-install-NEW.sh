...
function SNOOPYDOWNLOAD {
    ...
    #
    if [ "$SNOOPY_INSTALL_MODE" != "local" ]; then
...
case $ARG_INSTALL_MODE in
    ...
    local)
        SNOOPY_INSTALL_MODE="local"
        SNOOPY_PACKAGE_FILENAME=`/bin/ls -t1 snoopy-*.tar.gz | head -n1`
        ;;
...
#        ;;
    ...
    *)
        ...
        echo "- 'download'  ; downloads the latest stable version for 'local' off-line installation"
        echo "- 'local'     ; installs latest stable version from 'download' run on on-line system (above)"
...
if [ "$SNOOPY_INSTALL_MODE" == "git" ]; then
    ...
    if [ "$SNOOPY_INSTALL_MODE" == "local" ]; then
        ...
        SNOOPY_PACKAGE_VERSION=`echo $SNOOPY_PACKAGE_FILENAME | sed -e 's/^snoopy\-//' -e 's/\.tar\.gz$//'`
        echo "Version: $SNOOPY_PACKAGE_VERSION from $SNOOPY_PACKAGE_FILENAME" | tee -a $SNOOPY_INSTALL_LOGFILE
    fi
